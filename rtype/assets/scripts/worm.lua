function Update(vel, links, dt)
    ...
    if vel.life > 100 then
        if vel.posx < 1200 then
            vel.speed_x = 0
        elseif vel.posx > 1200 then
            vel.speed_x = -2
        end
    end
...
function Behavior(vel, links, time)
    ...
    if cycle % 6 == 0 then
        ...
        local command_1 = "shoot enem_weapon " .. x .. " " .. y .. " ../assets/scripts/ob_up_weapon_2.lua"
        local command_4 = "shoot enem_weapon " .. x .. " " .. y .. " ../assets/scripts/ob_down_weapon_2.lua"
    ...
    if cycle % 8 == 0 then
        ...
        local command_1 = "shoot target_weapon " .. x .. " " .. y .. " ../assets/scripts/target.lua"
        local command_4 = "shoot target_weapon " .. x .. " " .. y .. " ../assets/scripts/target.lua"
        table.insert(final_command, command)
        table.insert(final_command, command_1)
        table.insert(final_command, command_4)
    end
    if vel.life < 100 and cycle % 10 == 0 then
        local x = tostring(math.floor(vel.posx + 50))
        local y = tostring(math.floor(y_range))
        local command = "child Papillon " .. x .. " " .. y .. " ../assets/scripts/boul.lua"
        local command_1 = "child Papillon " .. x .. " " .. y .. " ../assets/scripts/boul.lua"
        local command_4 = "child Papillon " .. x .. " " .. y .. " ../assets/scripts/boul.lua"
        table.insert(final_command, command)
        table.insert(final_command, command_1)
        table.insert(final_command, command_4)
    end
